...
public class HealthModelServiceImpl extends ServiceImpl<HealthModelMapper, HealthModel> implements HealthModelService {
    ...
    public Result<String> saveEntity(HealthModel healthModel) {
        ...
        // 数据新增
        save(healthModel);
        return ApiResult.success("健康模组新增成功");
    }
    ...
    /**
     * 检验修改时权限 - 如果是公共模组，需要检验身份信息
     *
     * @param healthModel 健康模组
     */
    ...
    private void modelValidAuth(HealthModel healthModel) {
        // 看是谁人创建的模组（角色：LocalThreadHolder.getRole()） -> 公有模组？私人模组？
    ...
    private void validNormalValue(String normalValue) {
        ...
        AssertUtils.isTrue(normalValue.contains(","), "模组阈值非法，请重新输入");
    ...
    private void validParam(HealthModel healthModel) {
        ...
        AssertUtils.hasText(healthModel.getName(), "模组名不能为空");
        AssertUtils.hasText(healthModel.getDetail(), "模组介绍不能为空");
        AssertUtils.hasText(healthModel.getUnit(), "模组单位不能为空");
        AssertUtils.hasText(healthModel.getSymbol(), "模组符号不能为空");
        AssertUtils.hasText(healthModel.getNormalValue(), "模组正常阈值不能为空");
        AssertUtils.isTrue(healthModel.getName().length() < 100, "模组名称请控制在100字以内");
        AssertUtils.isTrue(healthModel.getDetail().length() < 200, "模组介绍请控制在200字以内");
    }
    ...
    public Result<String> updateEntity(HealthModel healthModel) {
        ...
        return ApiResult.success("健康模组修改成功");
    }
